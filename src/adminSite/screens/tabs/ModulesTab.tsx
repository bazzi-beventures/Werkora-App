...
import { useTenantSetting } from '../../../admin/configuration/useTenantSetting'
...
const MODULE_LABELS: Record<string, ModuleMeta> = {
  ...
  correction_request_push:{ label: 'Zeitkorrektur-Meldung', desc: 'Sofort-Push ans Büro (Admin, Management, Superadmin), wenn ein Mitarbeiter eine Zeitkorrektur beantragt. Ohne sie liegt der Antrag im Korrekturen-Screen, bis jemand von sich aus nachschaut. Erreicht nur Konten mit Mitarbeiter-Zeile — die Push geht an die Geräte der Person, nicht an das Konto (benötigt Zeiterfassung).', category: 'notifications', channel: 'push' },
...
export function ModulesTab({ view, tenantId }: { view: 'modules' | 'notifications'; tenantId: string }) {
  ...
  const {
    ...
    load: async () => {
      const result = await scoped.getModules(tenantId)
      setMeta({
      ...
      if (view === 'notifications') {
        try {
          const features = await scoped.getFeatures(tenantId)
    ...
    save: async (v) => {
      const result = await scoped.setModules(tenantId, [...v.enabled].sort(), [...v.beta].sort())
      return {
        enabled: [...result.enabled_modules].sort(),
        beta: [...(result.beta_modules ?? [])].sort(),
      }
  ...
  async function saveReminderTime(feature: string, time: string) {
    ...
    try {
      await scoped.setFeature(tenantId, feature, { time })
