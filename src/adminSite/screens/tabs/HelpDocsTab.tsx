...
import type { HelpDoc, ReindexStatus } from '../../../api/help'
...
export function HelpDocsTab({ tenantId }: { tenantId: string }) {
  ...
  async function loadDocs() {
    setLoading(true)
    try {
      setDocs(await scoped.listHelpDocs(tenantId))
      setModuleOff(false)
    } catch (e) {
  ...
  async function loadStatus() {
    try { setStatus(await scoped.getReindexStatus(tenantId)) } catch { /* Status ist optional */ }
  }
  ...
  async function onPick(e: ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true)
    try {
      await scoped.uploadHelpDoc(tenantId, file)
  ...
  async function onDelete(name: string) {
    ...
    try {
      await scoped.deleteHelpDoc(tenantId, name)
  ...
  async function onReindex() {
    setReindexing(true)
    try {
      await scoped.reindexHelp(tenantId)
