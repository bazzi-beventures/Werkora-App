...
export function TestingTab({ tenantId }: { tenantId: string }) {
  ...
  async function toggle(u: AuthUser) {
    ...
    setSavingId(u.id)
    try {
      // Nur das Häkchen — nicht Rolle, Status, Name und E-Mail gleich mit.
      // Der Betreiber kalibriert Tester, er verwaltet keine fremden Konten
      // (docs/specs/admin-werkora-ch.md §5.1).
      await scoped.setBetaTester(tenantId, u, next)
