...
import type { TravelCostRow } from '../../../api/admin'
...
export function TravelCostTab({ tenantId = null }: { tenantId?: string | null } = {}) {
  ...
  const {
    ...
    load: async () => {
      const res = await scoped.getTravelCost(tenantId)
      setDefaultTable(res.default_table)
      return fromResponse(res.travel_cost_table)
    },
    save: async (v) => {
      const res = await scoped.setTravelCost(tenantId, v.isDefault ? null : v.rows)
      return fromResponse(res.travel_cost_table)
    },
    onToast: showToast,
