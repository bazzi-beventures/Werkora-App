...
/**
 * Die Pille am Kopf der Karte — Fläche und Schrift aus den Tokens, nicht als
 * feste Hexwerte. Die alten Werte waren drei helle Pastelltöne mit dunkler
 * Schrift: im dunklen Theme leuchtende Flecken auf dem dunklen Kartengrund,
 * und in keinem Theme die Statusfarben, die der Rest der Seite führt.
 * `--*-soft`/`--*-ink` gibt es je Theme und sie sind gegeneinander
 * kontrastgeprüft (admin/tokens.css).
 */
function statusBadge(status: ServiceStatus | undefined): { text: string; bg: string; fg: string } {
  if (status === 'ok') return { text: 'ONLINE', bg: 'var(--success-soft)', fg: 'var(--success-ink)' }
  if (status === 'slow') return { text: 'LANGSAM', bg: 'var(--warning-soft)', fg: 'var(--warning-ink)' }
  if (status === 'down') return { text: 'OFFLINE', bg: 'var(--danger-soft)', fg: 'var(--danger-ink)' }
  return { text: 'KEIN CHECK', bg: 'var(--surface2)', fg: 'var(--text-muted)' }
}
...
function StatusCard({ name, data }: { name: ServiceName; data: ServiceHealth }) {
  ...
  const badge = stale
    ? { text: 'Probe stale', bg: 'var(--surface2)', fg: 'var(--text-muted)' }
    : statusBadge(status)
  // Bei stale: grauer Punkt — wir können dem letzten Status nicht mehr trauen.
  const dot = stale ? 'var(--text-muted)' : dotColor(status, data.uptime_24h.uptime_pct)
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        ...
        >
          <span className="admin-btn-spin-icon">↻</span>
